...
def aggregate_unique_emails(file_list):
    ...
    if unique_emails:
        ...
        print(f"[+] Created unique email list: {ALL_EMAILS_FILE} ({len(unique_emails)} unique emails)")

def main():
    parser = parse_args()
    extracted_files = [process_host(host.strip(), debug=parser.debug) for host in open(parser.list)] if parser.list else [process_host(parser.ip, debug=parser.debug)]
    aggregate_unique_emails([f for f in extracted_files if f])
